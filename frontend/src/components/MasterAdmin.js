...
const MasterAdmin = () => {
  ...
  const handleAssignAdminRole = async () => {
    try {
      ...
      const response = await axios.post('/api/setAdminRole', { uid }, {
  ...
  return (
    <Box sx={{ maxWidth: 800, mx: 'auto', mt: 4 }}>
      <Typography variant="h4" mb={2}>Master Admin Dashboard</Typography>
      ...
      <Box mb={4}>
        ...
        <TextField
          ...
          required
        />
        <Button variant="contained" onClick={handleAssignAdminRole} fullWidth sx={{ mt: 2, mb: 2 }}>
          Assign Admin Role
        </Button>
